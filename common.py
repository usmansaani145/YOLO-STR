...
class SPPm(nn.Module):
    def __init__(self, c1, c2, k=(5, 9, 13)):
        ...
        self.cv1 = nn.Conv2d(c1, c_, kernel_size=1)
        self.attention = ChannelAttention(c_)
        self.cv2 = nn.Conv2d(c_, c2, kernel_size=1)
        self.m = nn.ModuleList([nn.MaxPool2d(kernel_size=x, stride=1, padding=x // 2) for x in k])

    def forward(self, x):
        x = self.cv1(x)
        x = self.attention(x)
        x = self.cv2(torch.cat([x], 1))
        return x
...
class PyramidVisualTransformer(nn.Module):
    # Pyramid Visual Transformer
    def __init__(self, c, num_heads, num_layers):
        super(PyramidVisualTransformer, self).__init__()
        self.conv = nn.Conv2d(c, c, kernel_size=1, stride=1, padding=0, bias=False)
        self.linear = nn.Linear(c, c)  # learnable position embedding
        self.tr = nn.Sequential(*[TransformerLayer(c, num_heads) for _ in range(num_layers)])
        self.c = c
...
class BottleneckCSPCm(nn.Module):
    # Modified Bottleneck module with Pyramid Visual Transformer
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, num_heads=4, num_layers=1):
        super(BottleneckCSPCm, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = GhostConv(c1, c_, 1, 1)
        ...
        self.pvt = PyramidVisualTransformer(c_, num_heads, num_layers)
        self.cv4 = nn.Sequential(
            nn.Conv2d(2 * c_, 2 * c_, 3, 1, 1, groups=2 * c_, bias=False),
            nn.Conv2d(2 * c_, c2, 1, 1, 0, bias=False),
            nn.BatchNorm2d(c2),
            nn.ReLU(inplace=True)
        )
        self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)])
...
class BottleneckCSPCn(nn.Module):
    # CSP with Attention Bottleneck
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super(BottleneckCSPCn, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = nn.MultiheadAttention(c1, num_heads=1)
        self.cv2 = nn.MultiheadAttention(c1, num_heads=1)
        self.cv3 = nn.MultiheadAttention(c_, num_heads=1)
        self.cv4 = Conv(2 * c_, c2, 1, 1)
        self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)])

    def forward(self, x):
        y1, _ = self.cv3(self.cv1(x), x, x)
        y2, _ = self.cv2(x, x, x)
        return self.cv4(torch.cat((y1, y2), dim=1))
...
class STCSPCm(nn.Module):
    # CSP Bottleneck https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(STCSPCm, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 3, 1)
        self.cv2 = RobustConv(c1, c_, 3, 1)
        self.cv3 = Conv(c_, c_, 3, 1)
        self.cv4 = RobustConv(2 * c_, c2, 3, 1)
        num_heads = c_ // 32
        self.m = SwinTransformerBlock(c_, c_, num_heads, n)
        #self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)])
